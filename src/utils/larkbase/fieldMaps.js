export const PRODUCT_FIELD_MAP = {
  id: "Id",
  code: "Mã sản phẩm",
  name: "Sản phẩm",
  full_name: "Tên đầy đủ",
  category_name: "Tên danh mục",
  base_price: "Giá chung",
  inventories_cost: "Giá vốn",
  inventories_onHand: "Tồn kho",
  dealer_price: "Giá đại lý",
  distribution_price: "Giá phân phối",
  facebook_price: "Giá Facebook",
  tmdt_price: "Giá sàn TMĐT",
  ctv_price: "Giá CTV",
  is_active: "Đang bán",
  has_variants: "Có biến thể",
  hash: "hash",
};

export const PRODUCT_TYPE_MAP = {
  id: 2,
  code: 1,
  name: 1,
  full_name: 1,
  category_name: 1,
  base_price: 2,
  inventories_cost: 2,
  inventories_onHand: 2,
  dealer_price: 2,
  distribution_price: 2,
  facebook_price: 2,
  tmdt_price: 2,
  ctv_price: 2,
  is_active: 1,
  has_variants: 1,
  hash: 1,
};

export const PRODUCT_UI_TYPE_MAP = {
  id: "Number",
  code: "Text",
  name: "Text",
  full_name: "Text",
  category_name: "Text",
  base_price: "Currency",
  inventories_cost: "Currency",
  inventories_onHand: "Currency",
  dealer_price: "Currency",
  distribution_price: "Currency",
  facebook_price: "Currency",
  tmdt_price: "Currency",
  ctv_price: "Currency",
  is_active: "Text",
  has_variants: "Text",
  hash: "Text",
};

export const INVOICE_FIELD_MAP = {
  purchase_date: "Ngày thanh toán",
  id: "ID hoá đơn", // ID hoá đơn
  create_date: "Ngày tạo hoá đơn", // Ngày tạo hoá đơn
  code: "Mã hoá đơn", // Mã hoá đơn
  order_code: "Mã đơn hàng", // Mã đơn hàng

  payments_code: "Mã phiếu thu", // Mã thanh toán || Mã phiếu thu
  payments_id: "ID phiếu thu", // ID thanh toán || ID phiếu thu
  status_payments: "Trạng thái thanh toán",
  payment_amount: "Khách hàng thanh toán",
  payments_method: "Phương thức thanh toán",

  branch_id: "ID brand",
  sold_by_id: "ID nhân viên",
  sold_by_name: "Nhân viên",
  customer_code: "Mã khách hàng",
  customer_name: "Tên khách hàng",

  total_payment: "Tổng tiền thanh toán",
  total: "Tổng tiền",

  sur_value: "Phụ thu",

  status_value: "Trạng thái hoá đơn",
  partner_delivery_name: "Đơn vị vận chuyển",
  invoice_delivery_code: "Mã đơn GHN",
  cod: "COD",
  products_of_order: "Chi tiết đơn hàng",
  description: "Ghi chú",
  hash: "hash",
};

export const INVOICE_TYPE_MAP = {
  id: 1,
  create_date: 5,
  purchase_date: 5,
  code: 1,
  order_code: 1,

  payments_code: 1,
  payments_id: 1,
  status_payments: 1,
  payment_amount: 2,
  payments_method: 1,

  branch_id: 1,
  sold_by_id: 1,
  sold_by_name: 1,
  customer_code: 1,
  customer_name: 1,

  total_payment: 2,
  total: 2,

  sur_value: 2,

  status_value: 1,
  partner_delivery_name: 1,
  invoice_delivery_code: 1,
  cod: 1,
  products_of_order: 1,
  description: 1,
  hash: 1,
};

export const INVOICE_UI_TYPE_MAP = {
  id: "Text",
  create_date: "DateTime",
  purchase_date: "DateTime",
  code: "Text",
  order_code: "Text",

  payments_code: "Text",
  payments_id: "Text",
  status_payments: "Text",
  payment_amount: "Currency",
  payments_method: "Text",

  branch_id: "Text",
  sold_by_id: "Text",
  sold_by_name: "Text",
  customer_code: "Text",
  customer_name: "Text",

  total_payment: "Currency",
  total: "Currency",

  sur_value: "Currency",

  status_value: "Text",
  partner_delivery_name: "Text",
  invoice_delivery_code: "Text",
  cod: "Text",
  products_of_order: "Text",
  description: "Text",
  hash: "Text",
};

export const INVOICE_DETAIL_FIELD_MAP = {
  purchase_date: "Ngày thanh toán",
  id: "ID",
  order_id: "Order_ID",
  order_code: "Mã đơn hàng",
  invoice_id: "ID đơn hàng",
  invoice_code: "Mã hoá đơn",
  create_date: "Ngày tạo",
  status_value: "Trạng thái hoá đơn",
  sold_by_name: "Nhân viên",
  customer_code: "Mã khách hàng",
  customer_name: "Tên khách hàng",
  product_id: "Product ID",
  product_code: "Mã sản phẩm",
  product_name: "Tên sản phẩm",
  category_name: "Danh mục",
  cost_snapshot: "Giá vốn",
  quantity: "Số lượng",
  price: "Đơn giá",
  discount: "Giảm giá",
  discount_ratio: "Giảm giá theo %",
  sub_total: "Tổng giá trị",
  note: "Ghi chú",
  serial_numbers: "IMEI",
  return_quantity: "Số lượng hoàn",
  hash: "hash",
};

export const INVOICE_DETAIL_TYPE_MAP = {
  id: 1,
  order_id: 1,
  order_code: 1,
  invoice_id: 1,
  invoice_code: 1,
  purchase_date: 5,
  create_date: 5,
  status_value: 1,
  sold_by_name: 1,
  cost_snapshot: 2,
  customer_code: 1,
  customer_name: 1,
  product_id: 1,
  product_code: 1,
  product_name: 1,
  category_name: 1,
  quantity: 2,
  price: 2,
  discount: 2,
  discount_ratio: 2,
  sub_total: 2,
  note: 1,
  serial_numbers: 1,
  return_quantity: 2,
  hash: 1,
};

export const INVOICE_DETAIL_UI_TYPE_MAP = {
  id: "Text",
  order_id: "Text",
  order_code: "Text",
  invoice_id: "Text",
  invoice_code: "Text",
  purchase_date: "DateTime",
  create_date: "DateTime",
  status_value: "Text",
  sold_by_name: "Text",
  customer_code: "Text",
  customer_name: "Text",
  product_id: "Text",
  product_code: "Text",
  product_name: "Text",
  category_name: "Text",
  quantity: "Number",
  price: "Currency",
  discount: "Currency",
  discount_ratio: "Currency",
  sub_total: "Currency",
  note: "Text",
  serial_numbers: "Text",
  return_quantity: "Number",
  cost_snapshot: "Currency",
  hash: "Text",
};

export const CUSTOMER_FIELD_MAP = {
  id: "ID",
  created_date: "Ngày tạo",
  modified_date: "Ngày chỉnh sửa",
  name: "Tên khách hàng",
  code: "Mã khách hàng",
  phone_number: "Số điện thoại",
  address: "Địa chỉ",
  total_revenue: "Tổng doanh thu",
  total_invoiced: "Tổng số tiền đã xuất hóa đơn",
  debt: "Nợ hiện tại",
  total_point: "Tổng điểm",
  reward_point: "Điểm hiện tại",
  organization: "Tổ chức",
  sub_phone: "SĐT phụ",
  retailer_id: "retailerId",
  branch_id: "branchId",
  type: "type",
  comments: "Ghi chú",
  hash: "hash",
};

export const CUSTOMER_TYPE_MAP = {
  id: 1,
  created_date: 5,
  modified_date: 5,
  name: 1,
  code: 1,
  phone_number: 1,
  address: 1,
  total_revenue: 2,
  total_invoiced: 2,
  debt: 2,
  total_point: 2,
  reward_point: 2,
  organization: 1,
  sub_phone: 1,
  retailer_id: 1,
  branch_id: 1,
  type: 1,
  comments: 1,
  hash: 1,
};

export const CUSTOMER_UI_TYPE_MAP = {
  id: "Text",
  created_date: "DateTime",
  modified_date: "DateTime",
  name: "Text",
  code: "Text",
  phone_number: "Text",
  address: "Text",
  total_revenue: "Currency",
  total_invoiced: "Currency",
  debt: "Currency",
  total_point: "Number",
  reward_point: "Number",
  organization: "Text",
  sub_phone: "Text",
  retailer_id: "Text",
  branch_id: "Text",
  type: "Text",
  comments: "Text",
  hash: "Text",
};

export const RETURN_FIELD_MAP = {
  id: "Id",
  code: "Mã trả hàng",
  invoice_id: "ID hoá đơn",
  created_date: "Ngày tạo",
  return_date: "Ngày trả hàng",
  branch_id: "ID chi nhánh",
  branch_name: "Tên chi nhánh",
  return_total: "Tổng tiền trả hàng",
  received_by_id: "ID người nhận",
  sold_by_name: "Tên người bán",
  customer_id: "ID khách hàng",
  customer_code: "Mã khách hàng",
  customer_name: "Tên khách hàng",
  total_payment: "Tổng tiền khách trả",
  status_value: "Trạng thái",
  description: "Ghi chú",
  hash: "hash",
};

export const RETURN_TYPE_MAP = {
  id: 1,
  code: 1,
  invoice_id: 1,
  created_date: 5,
  return_date: 5,
  branch_id: 1,
  branch_name: 1,
  return_total: 2,
  total_payment: 2,
  received_by_id: 1,
  sold_by_name: 1,
  customer_id: 1,
  customer_code: 1,
  customer_name: 1,
  status_value: 1,
  description: 1,
  hash: 1,
};

export const RETURN_UI_TYPE_MAP = {
  id: "Text",
  code: "Text",
  invoice_id: "Text",
  created_date: "DateTime",
  return_date: "DateTime",
  branch_id: "Text",
  branch_name: "Text",
  return_total: "Currency",
  total_payment: "Currency",
  received_by_id: "Text",
  sold_by_name: "Text",
  customer_id: "Text",
  customer_code: "Text",
  customer_name: "Text",
  status_value: "Text",
  description: "Text",
  hash: "Text",
};

export const RETURN_DETAIL_FIELD_MAP = {
  id: "Id",
  code: "Mã phiếu trả hàng",
  invoice_id: "ID hoá đơn",
  created_date: "Ngày tạo",
  return_date: "Ngày trả hàng",
  status_value: "Trạng thái",
  product_code: "Mã sản phẩm",
  product_name: "Tên sản phẩm",
  quantity: "Số lượng",
  price: "Đơn giá",
  sub_total: "Thành tiền",
  branch_id: "ID chi nhánh",
  branch_name: "Tên chi nhánh",
  received_by_id: "ID người nhận",
  sold_by_name: "Tên người bán",
  customer_id: "ID khách hàng",
  customer_code: "Mã khách hàng",
  customer_name: "Tên khách hàng",
  hash: "hash",
};

export const RETURN_DETAIL_TYPE_MAP = {
  id: 1,
  code: 1,
  invoice_id: 1,
  created_date: 5,
  return_date: 5,
  status_value: 1,
  product_code: 1,
  product_name: 1,
  quantity: 2,
  price: 2,
  sub_total: 2,
  branch_id: 1,
  branch_name: 1,
  received_by_id: 1,
  sold_by_name: 1,
  customer_id: 1,
  customer_code: 1,
  customer_name: 1,
  hash: 1,
};

export const RETURN_DETAIL_UI_TYPE_MAP = {
  id: "Text",
  code: "Text",
  invoice_id: "Text",
  created_date: "DateTime",
  return_date: "DateTime",
  status_value: "Text",
  product_code: "Text",
  product_name: "Text",
  quantity: "Number",
  price: "Currency",
  sub_total: "Currency",
  branch_id: "Text",
  branch_name: "Text",
  received_by_id: "Text",
  sold_by_name: "Text",
  customer_id: "Text",
  customer_code: "Text",
  customer_name: "Text",
  hash: "Text",
};

export const CASHFLOW_FIELD_MAP = {
  id: "Id phiếu",
  create_date: "Ngày tạo",
  code: "Mã phiếu chi",
  branch_id: "ID chi nhánh",
  user_created: "Người tạo",
  amount: "Giá trị phiếu",
  status_value: "Trạng thái",
  created_by_id: "Id người tạo",
  cash_group_name: "cashGroup",
  description: "Ghi chú",
  method: "Phương thức thanh toán",
  partner_type: "Loại người nộp",
  cashflow_group_id: "Loại thu/chi",
  used_for_financial_reporting: "Hạch toán",
  partner_name: "Tên người nộp/nhận",
  retailer_id: "ID gian hàng",
  hash: "hash",
};

export const CASHFLOW_TYPE_MAP = {
  id: 1,
  create_date: 5,
  code: 1,
  branch_id: 1,
  user_created: 1,
  amount: 2,
  status_value: 1,
  created_by_id: 1,
  cash_group_name: 1,
  description: 1,
  method: 1,
  partner_type: 1,
  cashflow_group_id: 1,
  used_for_financial_reporting: 1,
  partner_name: 1,
  retailer_id: 1,
  hash: 1,
};

export const CASHFLOW_UI_TYPE_MAP = {
  id: "Text",
  create_date: "DateTime",
  code: "Text",
  branch_id: "Text",
  user_created: "Text",
  amount: "Currency",
  status_value: "Text",
  created_by_id: "Text",
  cash_group_name: "Text",
  description: "Text",
  method: "Text",
  partner_type: "Text",
  cashflow_group_id: "Text",
  used_for_financial_reporting: "Text",
  partner_name: "Text",
  retailer_id: "Text",
  hash: "Text",
};

export const SUPPLIER_FIELD_MAP = {
  id: "ID",
  code: "Mã NCC",
  createdDate: "Ngày tạo",
  name: "Tên nhà cung cấp",
  retailerId: "retailerId",
  branchId: "branchId",
  contactNumber: "Số điện thoại",
  address: "Địa chỉ",
  totalInvoiced: "Tổng mua",
  totalInvoicedWithoutReturn: "Tổng mua trừ trả hàng",
  debt: "Nợ NCC hiện tại",
  createdBy: "Nhân sự",
  isActive: "Đang hoạt động",
  hash: "hash",
};

export const SUPPLIER_TYPE_MAP = {
  id: 1,
  code: 1,
  createdDate: 5,
  name: 1,
  retailerId: 1,
  branchId: 1,
  contactNumber: 1,
  address: 1,
  totalInvoiced: 2,
  totalInvoicedWithoutReturn: 2,
  debt: 2,
  createdBy: 1,
  isActive: 1,
  hash: 1,
};

export const SUPPLIER_UI_TYPE_MAP = {
  id: "Text",
  code: "Text",
  createdDate: "DateTime",
  name: "Text",
  retailerId: "Text",
  branchId: "Text",
  contactNumber: "Text",
  address: "Text",
  totalInvoiced: "Currency",
  totalInvoicedWithoutReturn: "Currency",
  debt: "Currency",
  createdBy: "Text",
  isActive: "Text",
  hash: "Text",
};

export const PURCHASE_ORDER_FIELD_MAP = {
  id: "id",
  code: "Mã phiếu nhập",
  createdDate: "Ngày tạo phiếu",
  purchaseDate: "Ngày thanh toán",
  retailerId: "retailerId",
  branchId: "branchId",
  branchName: "Chi nhánh",
  discount: "Giảm giá",
  discountRatio: "Giảm giá theo %",
  total: "Tổng tiền",
  totalPayment: "Tổng tiền đã thanh toán",
  status: "Trạng thái",
  supplierId: "ID NCC",
  supplierCode: "Mã NCC",
  supplierName: "Tên nhà cung cấp",
  purchaseById: "ID người đặt",
  purchaseName: "Người đặt",
  purchase_detail_text: "Chi tiết phiếu nhập",
  payment_detail_text: "Chi tiết thanh toán",
  hash: "hash",
};

export const PURCHASE_ORDER_TYPE_MAP = {
  id: 1,
  code: 1,
  createdDate: 5,
  purchaseDate: 5,
  retailerId: 1,
  branchId: 1,
  branchName: 1,
  discount: 2,
  discountRatio: 2,
  total: 2,
  totalPayment: 2,
  status: 1,
  supplierId: 1,
  supplierCode: 1,
  supplierName: 1,
  purchaseById: 1,
  purchaseName: 1,
  purchase_detail_text: 1,
  payment_detail_text: 1,
  hash: 1,
};

export const PURCHASE_ORDER_UI_TYPE_MAP = {
  id: "Text",
  code: "Text",
  createdDate: "DateTime",
  purchaseDate: "DateTime",
  retailerId: "Text",
  branchId: "Text",
  branchName: "Text",
  discount: "Currency",
  discountRatio: "Number",
  total: "Currency",
  totalPayment: "Currency",
  status: "Text",
  supplierId: "Text",
  supplierCode: "Text",
  supplierName: "Text",
  purchaseById: "Text",
  purchaseName: "Text",
  purchase_detail_text: "Text",
  payment_detail_text: "Text",
  hash: "Text",
};

export const PURCHASE_ORDER_DETAIL_FIELD_MAP = {
  id: "id",
  code: "Mã phiếu nhập",
  status: "Trạng thái",
  createdDate: "Ngày tạo phiếu",
  purchaseById: "ID người đặt",
  purchaseName: "Người đặt",
  supplierId: "ID NCC",
  supplierCode: "Mã NCC",
  supplierName: "Tên nhà cung cấp",
  productCode: "Mã sản phẩm",
  productId: "ID sản phẩm",
  productName: "Tên sản phẩm",
  price: "Đơn giá",
  quantity: "Số lượng",
  discount: "Giảm giá",
  hash: "hash",
}

export const PURCHASE_ORDER_DETAIL_TYPE_MAP = {
  id: 1,
  code: 1,
  status: 1,
  createdDate: 5,
  purchaseById: 1,
  purchaseName: 1,
  supplierId: 1,
  supplierCode: 1,
  supplierName: 1,
  productCode: 1,
  productId: 1,
  productName: 1,
  price: 2,
  quantity: 2,
  discount: 2,
  hash: 1,
};
export const PURCHASE_ORDER_DETAIL_UI_TYPE_MAP = {
  id: "Text",
  code: "Text",
  status: "Text",
  createdDate: "DateTime",
  purchaseById: "Text",
  purchaseName: "Text",
  supplierId: "Text",
  supplierCode: "Text",
  supplierName: "Text",
  productCode: "Text",
  productId: "Text",
  productName: "Text",
  price: "Currency",
  quantity: "Number",
  discount: "Currency",
  hash: "Text",
}